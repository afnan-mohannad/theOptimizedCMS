                        ...
                        @if (isset($tags) && !empty($tags))
                            ...
                            <div class="card-body pt-0" wire:ignore>
                                ...
                                <select class="form-select mb-2" data-placeholder="{{__('admin.Select an option')}}" id="tags" multiple wire:model.live="tag_ids">
                                    <option value="{{null}}">{{__('admin.Select an option')}}</option>
                                    @foreach ($tags as $tag)
                                        <option value="{{$tag->id}}" wire:key="tag-{{ $tag->id }}">{{$tag->slug}}</option>
                                    @endforeach
                                </select>
                                <!--end::Select2-->
                                <!--end::Input group-->
                    ...
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-body pt-0" wire:ignore>
                            ...
                            <select data-control="select2" class="form-select mb-2" data-placeholder="{{__('admin.Select Status')}}" wire:model="status" id="status">
                                <option value="PUBLISHED" {{$status == "PUBLISHED" ? 'selected' : ""}}>{{__('admin.posts.PUBLISHED')}}</option>
                                <option value="DRAFT" {{$status == "DRAFT" ? 'selected' : ""}}>{{__('admin.posts.DRAFT')}}</option>
                                <option value="PENDING" {{$status == "PENDING" ? 'selected' : ""}}>{{__('admin.posts.PENDING')}}</option>
                            </select>
                            <!--end::Select2-->
                            <!--begin::Description-->
                            ...
                            <div class="menu-item px-3 mt-3">
                                <div class="menu-content px-3">
                                    <label class="form-check form-switch form-check-custom form-check-solid">
                                        <input class="form-check-input" type="checkbox" wire:model="is_active" id="is_active" {{$is_active == 1 ? 'checked' : ""}}/>
                        ...
                        <div class="card-header">
                            ...
                            <div class="menu-item px-3 mt-3">
                                <div class="menu-content px-3">
                                    <label class="form-check form-switch form-check-custom form-check-solid">
                                        <input class="form-check-input" type="checkbox" wire:model="featured" id="featured" {{$featured == 1 ? 'checked' : ""}}/>
                                        <span class="form-check-label text-muted fs-7"></span>
                                    </label>
                                </div>
                            </div>
                    ...
                    <div class="fv-row mb-7">
                        ...
                        <div class="card card-flush py-4">
                            ...
                            <div class="card-header">
                                <!--begin::Card title-->
                                <div class="card-title">
                                    <h2>{{__('admin.banners.Picture')}}</h2>
                                </div>
                                <!--end::Card title-->
                            </div>
                            <!--end::Card header-->
                            ...
                            <div class="card-body text-center pt-0">
                                ...
                                >
                                    ...
                                    @if ($post->picture)
                                        <style>
                                        .image-input-placeholder-picture { background-image: url('{{ asset('storage/'.$post->picture) }}'); }
                                        </style>
                                    @endif
                                    <!--end::Image input placeholder-->
                                    <a href="{{asset('storage/'. $post->picture)}}" target="_blank">
                                        <div class="image-input image-input-empty image-input-outline image-input-placeholder image-input-placeholder-picture mb-3" id="kt_picture_input" data-kt-image-input="true" wire:ignore>
                                    ...
                                    <!--begin::delete-->
                                    <center wire:ignore>
                                        <a class="btn btn-icon btn-circle w-25px h-25px bg-body shadow mb-5" wire:click="removePicture"
                                        id="delete_picture">
                            ...
                            <div class="card-body text-center pt-0">
                                ...
                                >
                                    ...
                                    @if ($post->cover_picture)
                                        <style>
                                        .image-input-placeholder-cover-picture { background-image: url('{{ asset('storage/'.$post->cover_picture) }}'); } [data-theme="dark"] .image-input-placeholder-cover-picture { background-image: url('{{ asset('storage/'.$post->cover_picture) }}'); }
                                        </style>
                                    @endif
                                    <!--end::Image input placeholder-->
                                    <a href="{{asset('storage/'. $post->cover_picture)}}" target="_blank">
                                        <div class="image-input image-input-empty image-input-outline image-input-placeholder image-input-placeholder-cover-picture mb-3" id="kt_cover_picture_input" data-kt-image-input="true" wire:ignore>
                                    ...
                                    <!--begin::Remove-->
                                    <center wire:ignore>
                                        <a class="btn btn-icon btn-circle w-25px h-25px bg-body shadow mb-5" wire:click="removeCoverPicture"
                                        id="delete_cover_picture">
                                            <i class="bi bi-trash fs-2 red"></i>
                                        </a>
                                    </center>
                                    <!--end::Remove-->
                                ...
                                <div class="card card-flush py-4">
                                    ...
                                    <div class="card-body pt-0" wire:ignore>
                                        ...
                                        is-invalid @enderror" wire:model="locale" id="locale" data-placeholder="{{__('admin.Select Locale')}}">
                                            <option value="{{null}}">{{__('admin.Select an option')}}</option>
                                            <option value="en" {{$locale=="en" ? 'selected' : ""}}>{{__('admin.English')}}</option>
                                            <option value="ar" {{$locale=="ar" ? 'selected' : ""}}>{{__('admin.Arabic')}}</option>
                                        </select>
                                        <!--end::Select2-->
                                        <!--begin::Description-->
                                    ...
                                    <div class="card-body pt-0">
                                        ...
                                        <div class="mb-10 fv-row">
                                            <!--begin::Label-->
                                            <label class="form-label">{{__('admin.Excerpt')}}</label>
                                            <!--end::Label-->
                                            <!--begin::Input-->
                                            <textarea class="form-control mb-2 @error('excerpt') is-invalid @enderror form-control-solid" rows="3" wire:model="excerpt"></textarea>
                                            <!--end::Input-->
                                            <!--begin::Description-->

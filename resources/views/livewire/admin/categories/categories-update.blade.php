<x-update-modal title="{{__('admin.categories.Update Category')}}">
    <!--begin::Input group-->
    <div class="fv-row mb-7">
        <div class="col-12">
            ...
            @if ($picture && $picture != null)
                @if(str_starts_with($picture, 'categories'))
                    <div class="my-4">
                        <img src="{{ asset('storage/'.$picture) }}" width="100%" height="150px" class="picture-style">
                    </div>
                @else
                    <div class="my-4">
                        <img src="{{ $picture->temporaryUrl() }}" width="100%" height="150px" class="picture-style">
                    </div>
                @endif
            @endif
            @if ($picture)
    ...
    <div class="row mb-7">
        <div class="col-12">
            ...
            <!--end::Label-->
            <!--begin::Input-->
            <input type="text" wire:model="name_en" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="{{__('admin.Name')}} {{ __('admin.(en)') }}" />
            <!--end::Input-->
    ...
    <div class="row mb-7">
        <div class="col-12">
            ...
            <!--end::Label-->
            <!--begin::Input-->
            <input type="text" wire:model="slug" class="form-control form-control-solid mb-3 mb-lg-0 disabled" placeholder="{{__('admin.Slug')}}" readonly/>
            <!--end::Input-->
    ...
    @if($parent_id)
        ...
        <div class="fv-row mb-5 mt-5">
            @if(isset($categories) && !empty($categories))
                <div class="row mb-7" id="categories_select_drop_down" wire:ignore>
                    <div class="col-12">
                        <!--begin::Label-->
                        ...
                        <!--end::Label-->
                        <select class="form-select form-select-solid update_parent_id" data-control="select2" data-placeholder="{{__('admin.Select category')}}" data-allow-clear="true" wire:model="parent_id" id="update_parent_id">
                            <option value="">{{__('admin.Select category')}}</option>
                            @foreach ($categories as $category)
                                <option value="{{$category->id}}" @if(isset($parent_id) && $parent_id == $category->id) selected="selected" @endif>{{$category->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
{{-- @script()
    <script>
        $(document).ready(function() {
            ...
            $('#update_parent_id').on('change', function() {
                let data = $(this).val();
                @this.update_parent_id = data;
            });
        });
    </script>
@endscript --}}